...
;* Date/Time created: Sat Aug 17 16:09:06 2019                 *
;***************************************************************
...
;*** 134	-----------------------    asm(" EDIS");
;*** 136	-----------------------    IER |= 0x3001u;
;*** 138	-----------------------    *(&PieCtrlRegs+2L) |= 0x40u;
;*** 139	-----------------------    *(&PieCtrlRegs+2L) |= 0x20u;
;*** 141	-----------------------    ConfigCpuTimer(&CpuTimer0, 100.0F, 25.0F);
;*** 142	-----------------------    ConfigCpuTimer(&CpuTimer2, 100.0F, 500.0F);
;*** 144	-----------------------    asm(" clrc INTM");
;*** 145	-----------------------    asm(" clrc DBGM");
;*** 145	-----------------------    return;
	.dwcfa	0x1d, -2
 ...
 EDIS
	...
	.dwpsn	"DSP280x_CpuTimers.c",138,2
        ...
        OR        @_PieCtrlRegs+2,#0x0040 ; |138| 
	.dwpsn	"DSP280x_CpuTimers.c",139,2
        OR        @_PieCtrlRegs+2,#0x0020 ; |139| 
	.dwpsn	"DSP280x_CpuTimers.c",141,2
        MOV       AL,#0
        MOV       AH,#16840
        MOVL      *-SP[2],ACC           ; |141| 
        SPM       #0
        MOVL      XAR4,#_CpuTimer0      ; |141| 
        MOV       AH,#17096
        LCR       #_ConfigCpuTimer      ; |141| 
        ; call occurs [#_ConfigCpuTimer] ; |141| 
	.dwpsn	"DSP280x_CpuTimers.c",142,2
        MOV       AL,#0
        MOV       AH,#17402
        MOVL      *-SP[2],ACC           ; |142| 
        MOVL      XAR4,#_CpuTimer2      ; |142| 
        MOV       AH,#17096
 ...
 clrc DBGM
	.dwpsn	"DSP280x_CpuTimers.c",147,1
        SUBB      SP,#2
	.dwcfa	0x1d, -2
        SPM       #0
        LRETR
	...
	.dwattr DW$19, DW_AT_end_line(0x93)
